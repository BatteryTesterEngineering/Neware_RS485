...
# This file is the API for the constant_current_discharge_request message.

from enum import IntEnum
from rs485.messages.message_helpers import real_to_bus_value, bus_to_real_value

class constant_current_discharge_request_message_current_range(IntEnum):
    LOW_RANGE = int(0)
    MID_RANGE = int(1)
    HIGH_RANGE = int(2)

class constant_current_discharge_request():
    ...
    def __get_current_parameters(args, payload):
        ...
        if payload["current_range"] == constant_current_discharge_request_message_current_range.LOW_RANGE:
            # Get the divisor for LOW_RANGE
            current_parameters_dict["divisor"] = 16128.0
        elif payload["current_range"] == constant_current_discharge_request_message_current_range.MID_RANGE:
            # Get the divisor for MID_RANGE
            current_parameters_dict["divisor"] = 2688.0
        elif payload["current_range"] == constant_current_discharge_request_message_current_range.HIGH_RANGE:
    ...
    def __get_current_range_enum(self, payload):
        ...
        if payload["current"] <= 1.0:
            return constant_current_discharge_request_message_current_range.LOW_RANGE
        elif payload["current"] <= 6.0:
            return constant_current_discharge_request_message_current_range.MID_RANGE
        elif payload["current"] <= 12.0:
            return constant_current_discharge_request_message_current_range.HIGH_RANGE
        else:
    ...
    def unpack_message(self, message_bytes):
        ...
        message_payload['current_range'] = constant_current_discharge_request_message_current_range(current_range_bus_value)
