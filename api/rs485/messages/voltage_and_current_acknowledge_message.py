...
# This file is the API for the voltage_and_current_acknowledge message.

from enum import IntEnum
from rs485.messages.message_helpers import real_to_bus_value, bus_to_real_value

class voltage_and_current_acknowledge_message_current_range(IntEnum):
    LOW_RANGE = int(0)
    MID_RANGE = int(1)
    HIGH_RANGE = int(2)

class voltage_and_current_acknowledge_message_channel_status(IntEnum):
    CHANNEL_ACTIVE = int(0)
    CHANNEL_INVALID = int(1)
    CHANNEL_REST = int(6)

class voltage_and_current_acknowledge():
    ...
    def __get_current_parameters(args, payload):
        ...
        if payload["current_range"] == voltage_and_current_acknowledge_message_current_range.LOW_RANGE:
            # Get the divisor for LOW_RANGE
            current_parameters_dict["divisor"] = 16128.0
        elif payload["current_range"] == voltage_and_current_acknowledge_message_current_range.MID_RANGE:
            # Get the divisor for MID_RANGE
            current_parameters_dict["divisor"] = 2688.0
        elif payload["current_range"] == voltage_and_current_acknowledge_message_current_range.HIGH_RANGE:
    ...
    def __get_current_range_enum(self, payload):
        ...
        if payload["current"] <= 1.0:
            return voltage_and_current_acknowledge_message_current_range.LOW_RANGE
        elif payload["current"] <= 6.0:
            return voltage_and_current_acknowledge_message_current_range.MID_RANGE
        elif payload["current"] <= 12.0:
            return voltage_and_current_acknowledge_message_current_range.HIGH_RANGE
        else:
    ...
    def pack_message(self, message_bytes, payload):
        ...
        message_bytes[8:12] = current_bus_value.to_bytes(4, byteorder='little', signed=True)
        message_bytes[33] = payload["current_range"]
        message_bytes[35] = payload["channel_status"]
    ...
    def unpack_message(self, message_bytes):
        ...
        current_bus_value = int.from_bytes(message_bytes[8:12], byteorder='little', signed=True)
        current_range_bus_value = message_bytes[33]
        channel_status_bus_value = message_bytes[35]
        ...
        message_payload['current_range'] = voltage_and_current_acknowledge_message_current_range(current_range_bus_value)
        message_payload['channel_status'] = voltage_and_current_acknowledge_message_channel_status(channel_status_bus_value)
