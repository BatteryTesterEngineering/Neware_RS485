...
# This file is the API for the constant_power_charge_request message.

from enum import IntEnum
from rs485.messages.message_helpers import real_to_bus_value, bus_to_real_value

class constant_power_charge_request_message_power_range(IntEnum):
    LOW_RANGE = int(0)
    MID_RANGE = int(1)
    HIGH_RANGE = int(2)

class constant_power_charge_request():
    ...
    def __get_power_parameters(args, payload):
        ...
        if payload["power_range"] == constant_power_charge_request_message_power_range.LOW_RANGE:
            # Get the divisor for LOW_RANGE
            power_parameters_dict["divisor"] = 1.0
        elif payload["power_range"] == constant_power_charge_request_message_power_range.MID_RANGE:
            # Get the divisor for MID_RANGE
            power_parameters_dict["divisor"] = 1.0
        elif payload["power_range"] == constant_power_charge_request_message_power_range.HIGH_RANGE:
    ...
    def __get_power_range_enum(self, payload):
        ...
        if payload["power"] <= 0.0:
            return constant_power_charge_request_message_power_range.LOW_RANGE
        elif payload["power"] <= 0.0:
            return constant_power_charge_request_message_power_range.MID_RANGE
        elif payload["power"] <= 60.0:
            return constant_power_charge_request_message_power_range.HIGH_RANGE
        else:
    ...
    def unpack_message(self, message_bytes):
        ...
        message_payload['power_range'] = constant_power_charge_request_message_power_range(power_range_bus_value)
